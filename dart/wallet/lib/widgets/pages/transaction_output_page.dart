    ...
    if (output.value.hasAccountRegistration()) {
        ...
        body.add(OverUnder(
            ...
            under: TappableLink(
              ...
              child: SizedBox.square(
                  dimension: 32,
                  child: BitMapViewer.forLockAddress(
                      registration.associationLock)),
            )));
      }
      ...
      return OverUnder(
          over: const Text("Registration",
              style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
          under: Wrap(
              children: body.intersperse(const VerticalDivider()).toList()));
    } else {
      return Container();
    }
  }
            ...
            children: [
              ...
              OverUnder(
                  over: const Text("Data",
                      style:
                          TextStyle(fontSize: 14, fontWeight: FontWeight.bold)),
                  under: Text(vertex.data.toProto3Json().toString(),
                      style: const TextStyle(
                          fontSize: 14, color: Colors.blueGrey))),
            ],
          ),
            ...
            children: [
              ...
              OverUnder(
                  over: const Text("Data",
                      style:
                          TextStyle(fontSize: 14, fontWeight: FontWeight.bold)),
                  under: Text(edge.data.toProto3Json().toString(),
                      style: const TextStyle(
                          fontSize: 14, color: Colors.blueGrey))),
            ],
          ),
...
class TransactionOutputIdCard extends StatelessWidget {
  const TransactionOutputIdCard(
      {super.key, required this.reference, this.scale = 1.0});
