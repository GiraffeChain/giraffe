...
class Ed25519VRF {
  ...
  private val impl = new ECVRF25519Impl
  impl.precompute()
...
private[crypto] class ECVRF25519Impl extends EC {
  ...
  private val neutralPointBytes: Array[Byte] =
    Array.fill(POINT_BYTES)(0x00.toByte)
  ...
  private[crypto] def verifyKeyPair(sk: Array[Byte], pk: Array[Byte]): Boolean =
  ...
  private[crypto] def vrfProof(
    ...
    val H: (PointAccum, Array[Byte]) =
      ECVRF_hash_to_curve_try_and_increment(pk, alpha)
  ...
  private[crypto] def vrfVerify(
    ...
    val c = pi.slice(POINT_BYTES, POINT_BYTES + C_BYTES) ++ Array.fill(
      SCALAR_BYTES - C_BYTES
    )(0x00.toByte)
  ...
  private[crypto] def vrfProofToHash(pi: Array[Byte]): Array[Byte] = {
    //    assert(pi.length == PI_BYTES)
    val gamma_str = pi.take(POINT_BYTES)
  ...
  private def ECVRF_hash_to_curve_try_and_increment(
      Y: Array[Byte],
      a: Array[Byte]
  ): (PointAccum, Array[Byte]) = {
    var ctr = 0
   ...
   */
  private def ECVRF_nonce_generation_RFC8032(
      sk: Array[Byte],
      h: Array[Byte]
  ): Array[Byte] = {
    val out: Array[Byte] = new Array[Byte](sha512Digest.getDigestSize)
