...
pub async fn parent_of(connection: &Connection, id: BlockId) -> Option<BlockId> {
    let id = id.clone();
    connection
        .call(|conn| {
            let mut stmt =
...
pub async fn find_common_ancestor(
    ...
    let mut a = a;
    let mut b = b;
    let mut a_height = get_height(connection, a.clone()).await;
    let mut b_height = get_height(connection, a.clone()).await;
    let mut a_path = vec![a.clone()];
    let mut b_path = vec![b.clone()];
    while a_height > b_height {
        a = parent_of(connection, a.clone()).await.unwrap();
        a_path.push(a.clone());
        a_height -= 1;
    }
    while b_height > a_height {
        b = parent_of(connection, b.clone()).await.unwrap();
        b_path.push(b.clone());
        b_height -= 1;
    }
    while a != b {
