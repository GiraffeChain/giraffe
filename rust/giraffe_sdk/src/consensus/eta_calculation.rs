...
pub struct EtaCalculation(Vec<u8>, Clock, Connection);

impl EtaCalculation {
    pub async fn next_eta(&self, parent_slot_id: SlotId, child_slot: i64) -> Vec<u8> {
        let child_epoch = self.1.epoch_of(child_slot);
        if child_epoch == 0 {
            return self.0.clone();
        } else {
            let parent_epoch = self.1.epoch_of(parent_slot_id.slot as i64);
            let parent_header = data::fetch_header(&self.2, parent_slot_id.block_id.unwrap())
                .await
                .unwrap();
            if parent_epoch == child_epoch {
                ...
                let two_thirds_length = 2 * self.1.epoch_length_slots / 3;
                let mut two_thirds_best = parent_header;
                while two_thirds_best.slot % self.1.epoch_length_slots > two_thirds_length {
                    two_thirds_best =
                        data::fetch_header(&self.2, two_thirds_best.parent_header_id.unwrap())
                            .await
                            .unwrap();
                }
    ...
    async fn calculate(&self, two_thirds_best: &BlockHeader) -> Vec<u8> {
        let epoch = self.1.epoch_of(two_thirds_best.slot as i64);
        let (epoch_start, _) = self.1.epoch_range(epoch);
        let mut epoch_data: Vec<BlockHeader> = Vec::new();
        epoch_data.push(two_thirds_best.clone());
        let mut cont = false;
        let parent = data::fetch_header(&self.2, two_thirds_best.parent_header_id.clone().unwrap())
            .await
            .unwrap();
        ...
        while cont {
            let parent =
                data::fetch_header(&self.2, two_thirds_best.parent_header_id.clone().unwrap())
                    .await
                    .unwrap();
            if parent.slot >= (epoch_start as u64) {
                epoch_data.push(parent.clone());
